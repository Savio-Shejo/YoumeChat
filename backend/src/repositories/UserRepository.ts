...
export class UserRepository {
  ...
  public async searchUsers(query: string, currentUserId: string, limit: number = 20, page: number = 1): Promise<{ users: IUser[]; total: number }> {
    ...
    const [users, total] = await Promise.all([
      User.find(filter).select('-blockedUsers').skip(skip).limit(limit).exec(),
  ...
  public async blockUser(userId: string, targetUserId: string): Promise<IUser | null> {
    return User.findByIdAndUpdate(
      userId,
  ...
  public async unblockUser(userId: string, targetUserId: string): Promise<IUser | null> {
    return User.findByIdAndUpdate(
      userId,
