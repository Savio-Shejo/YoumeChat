...
export class ChatRepository {
  ...
  public async getUserChats(userId: string, limit: number = 20, page: number = 1): Promise<{ chats: IChat[]; total: number }> {
  ...
  public async togglePin(chatId: string, userId: string): Promise<IChat | null> {
    ...
    const exists = chat.pinnedBy.some((id) => id.equals(uId));

    if (exists) {
      chat.pinnedBy = chat.pinnedBy.filter((id) => !id.equals(uId));
    } else {
  ...
  public async toggleMute(chatId: string, userId: string): Promise<IChat | null> {
    ...
    const uId = new Types.ObjectId(userId);
    const exists = chat.isMutedBy.some((id) => id.equals(uId));

    if (exists) {
      chat.isMutedBy = chat.isMutedBy.filter((id) => !id.equals(uId));
    } else {
  ...
  public async toggleArchive(chatId: string, userId: string): Promise<IChat | null> {
    ...
    const uId = new Types.ObjectId(userId);
    const exists = chat.isArchivedBy.some((id) => id.equals(uId));

    if (exists) {
      chat.isArchivedBy = chat.isArchivedBy.filter((id) => !id.equals(uId));
    } else {
